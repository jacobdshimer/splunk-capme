  ...
  $("#logout").click(function(event) {
    ...
    function reqCap(caller) {
        ...
        if ($(".capme_submit").html() == "submit") {
            ...
            // Splunk ID
            var spid = s2h(($("#spid").val()));

            // Source Time
            var stime = s2h(($("#stime").val()));

            // Source Time
            var stype = s2h(($("#stype").val()));

            // Transcript
            var xscript = s2h($('input:radio[name=xscript]:checked').val());

            // SID Source
            var sidsrc = s2h("splunk");
        ...
        // Max TX
            ...
            if (err == 0) {
                ...
                var urArgs = "d=" + spid + "-" + stime + "-" + stype + "-" + maxtx + "-" + sidsrc + "-" + xscript;

                $(function(){
                    $.get(".inc/callback-splunk.php?" + urArgs, function(data){cbtx(data)});
                });
